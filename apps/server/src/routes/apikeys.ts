...
import { getDb, createTables, queryAll, queryOne, execute } from '@expense/expense-service'
  ...
  try {
    ...
    const rows = await queryAll(
      ...
      `SELECT id, label, created_at, last_used_at, revoked_at,
              LEFT('exp_••••' || LEFT(key_hash, 4), 12) AS key_preview
       FROM api_keys
       WHERE user_id = ?
...
router.post('/', validate(createKeySchema), async (req, res) => {
  try {
    ...
    await execute(db, 'INSERT INTO api_keys (id, user_id, key_hash, label) VALUES (?, ?, ?, ?)', [
...
router.delete('/:id', async (req, res) => {
  try {
    const db = await getDb()
    await createTables(db)
    const existing = await queryOne(db, 'SELECT id FROM api_keys WHERE id = ? AND user_id = ? AND revoked_at IS NULL', [
    ...
    await execute(db, 'UPDATE api_keys SET revoked_at = NOW() WHERE id = ?', [req.params.id as string])
