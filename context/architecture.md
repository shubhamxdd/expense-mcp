...
- **SQLite (`server/db/`)**: Auth and metadata only.
  ...
  - `google_tokens` — user_id (FK), refresh_token, access_token, expires_at
...
- Web app: User signs in via Google OAuth. Backend exchanges auth code for tokens, stores refresh token (encryption planned — currently stored as-is). Issues a JWT session token to the frontend. Every subsequent API request authenticates via JWT.
